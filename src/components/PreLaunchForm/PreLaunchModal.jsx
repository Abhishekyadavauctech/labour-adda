...
export default function PreLaunchModal() {
  ...
  useEffect(() => {
    fetch(`https://api.partyhubconnect.com/api/categories?isPagination=false`) 
      .then(r => r.json())
  ...
  useEffect(() => {
    ...
    fetch(`https://api.partyhubconnect.com/api/subcategories?isPagination=false&category=${selectedCategory}`)
      .then(r => r.json())
      .then(d => {
        if (d.success)
          setSubCategories(d.data.subCategories);
      });
  }, [selectedCategory]);
    ...
    try {
      const results = await Promise.all(
        Array.from(files).map(async file => {
          ...
          const res = await fetch("https://api.partyhubconnect.com/api/upload/uploadImage", { method: "POST", body: fd });
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("https://api.partyhubconnect.com/api/prelaunch-register/", {
        method: "POST",
                    ...
                    <div className="form-section">
                      <div className="form-grid-2">
                        <Field label="Full Name" required>
                          <input className="inp" placeholder="e.g. John Smith" value={name} onChange={e => setName(e.target.value)} />
                        </Field>
                    ...
                    <div className="form-section">
                      ...
                      <div className="form-grid-2">
                        ...
                        <Field label="Business Phone">
                          <input className="inp" placeholder=" 416-555-XXXX" value={businessPhone} maxLength={13} onChange={e => setBizPhone(e.target.value.replace(/[^0-9+]/g, ""))} />
                        </Field>
                      </div>
